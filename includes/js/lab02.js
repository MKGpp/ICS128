...
function displayOccupancy() {
    ...
    let percent = document.getElementById("displayOccupancy").value;


    if (percent < 0 || percent > 100) {
        alert("Incorrect - not between 0-100");
    } else {
        let result = document.getElementById("occupancy");
        result.innerHTML = `The Hotel is:<br>`;
        result.innerHTML += `<span id="percent">${percent}%</span> booked!`;

        let percentColor = document.getElementById("percent")
